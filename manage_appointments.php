...
// Handle appointment deletion if requested
...
// Fetch all appointments safely without complex joins that cause column errors
$appointments_result = $conn->query("SELECT * FROM appointments ORDER BY id DESC");
?>
...
<head>
    ...
    <title>Appointments Queue - Medi Lanka HMS</title>
    <!-- Google Fonts -->
    ...
    <div class="main-wrapper">
        ...
        <header class="top-navbar">
            <div class="d-flex align-items-center gap-2">
                <h5 class="fw-bold text-dark mb-0">Appointments Queue</h5>
            </div>
        ...
        <div class="p-4 p-md-5">
            <div class="content-card mb-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 class="fw-bold text-dark mb-1">Scheduled Clinical Appointments</h4>
                        <p class="text-secondary small mb-0">Monitor and manage patient-doctor consultations queue.</p>
                    </div>
                    <a href="add_appointment.php" class="btn btn-primary fw-semibold px-4 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2">
                                ...
                                <?php while ($row = $appointments_result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td class="fw-semibold text-dark">
                                            <?php 
                                                // Safely display patient name or ID
                                                if (isset($row['patient_name'])) {
                                                    echo htmlspecialchars($row['patient_name']);
                                                } elseif (isset($row['name'])) {
                                                    echo htmlspecialchars($row['name']);
                                                } else {
                                                    echo 'Patient #' . ($row['patient_id'] ?? $row['id']);
                                                }
                                            ?>
                                        </td>
                                        <td>
                                            <?php 
                                                // Safely display doctor name or ID
                                                if (isset($row['doctor_name'])) {
                                                    echo 'Dr. ' . htmlspecialchars($row['doctor_name']);
                                                } else {
                                                    echo 'Doctor #' . ($row['doctor_id'] ?? 'N/A');
                                                }
                                            ?>
                                        </td>
                                        <td><?php echo htmlspecialchars($row['date'] ?? $row['appointment_date'] ?? 'N/A'); ?></td>
                                        <td><span class="badge bg-warning bg-opacity-10 text-dark px-2.5 py-1.5 fw-semibold"><?php echo htmlspecialchars($row['status'] ?? 'Scheduled'); ?></span></td>
                                        <td class="text-end">
                                            <a href="edit_appointment.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-light border text-primary me-1"><i class="bi bi-pencil"></i></a>
                                            <a href="manage_appointments.php?delete=<?php echo $row['id']; ?>" class="btn btn-sm btn-light border text-danger" onclick="return confirm('Are you sure you want to delete this appointment?');"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="text-center py-5 text-muted small">
                                        ...
                                        No appointment records found in the database.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
