...
// Fetch appointments with correct appointment_date
$appointments_result = $conn->query("SELECT id, appointment_date as date, status FROM appointments ORDER BY id DESC LIMIT 5");

// Fetch prescriptions with correct medication column
$prescriptions_result = $conn->query("SELECT id, medication as details FROM prescriptions ORDER BY id DESC LIMIT 5");
?>
...
<head>
    ...
    <title>Patient Portal - Medi Lanka HMS</title>
...
<body>
    <nav class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="patient_portal.php" class="sidebar-link active"><i class="bi bi-person-workspace"></i> Patient Portal</a>
    ...
    <div class="main-wrapper">
        <header class="top-navbar">
            <h5 class="fw-bold text-dark mb-0">Patient Self-Service Portal</h5>
            <div class="d-flex align-items-center gap-4">
                <div class="d-flex align-items-center gap-2 bg-white px-3.5 py-2 rounded-pill border shadow-sm">
                    ...
                    <span class="text-secondary small fw-medium">User: <strong class="text-dark"><?php echo htmlspecialchars($_SESSION['username'] ?? 'admin'); ?></strong></span>
                </div>
            ...
            <div class="row g-4 mb-4">
                <div class="col-lg-7">
                    <div class="content-card h-100">
                        <h5 class="fw-bold text-dark mb-3"><i class="bi bi-calendar-check text-primary me-2"></i>My Recent Appointments</h5>
                                    ...
                                    <?php if ($appointments_result && $appointments_result->num_rows > 0): ?>
                                        <?php while ($app = $appointments_result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td><span class="badge bg-success bg-opacity-10 text-success px-2 py-1"><?php echo htmlspecialchars($app['status']); ?></span></td>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php else: ?>
                                        <tr><td colspan="3" class="text-center text-muted py-3 small">No appointments found.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="col-lg-5">
                    <div class="content-card h-100">
                        <h5 class="fw-bold text-dark mb-3"><i class="bi bi-capsule text-primary me-2"></i>My Prescriptions</h5>
                        <div class="table-responsive">
                            <table class="table align-middle mb-0">
                                <thead class="table-light text-secondary uppercase small">
                                ...
                                <tbody>
                                    <?php if ($prescriptions_result && $prescriptions_result->num_rows > 0): ?>
                                        <?php while ($pres = $prescriptions_result->fetch_assoc()): ?>
                                            <tr>
                                                <td class="fw-bold text-primary">#<?php echo $pres['id']; ?></td>
                                                <td><?php echo htmlspecialchars($pres['details']); ?></td>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php else: ?>
                                        <tr><td colspan="2" class="text-center text-muted py-3 small">No prescriptions found.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
