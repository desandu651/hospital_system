...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM laboratory WHERE id = $id");
    header("Location: laboratory.php");
    exit();
}

// Check if laboratory table exists and fetch records
...
<head>
    ...
    <title>Laboratory Tests - Medi Lanka HMS</title>
...
<body>
    <nav class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="laboratory.php" class="sidebar-link active"><i class="bi bi-eyedropper"></i> Laboratory Tests</a>
    ...
    <div class="main-wrapper">
        <header class="top-navbar">
            <h5 class="fw-bold text-dark mb-0">Laboratory Tests & Results</h5>
        ...
        <div class="p-4 p-md-5">
            <div class="content-card mb-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 class="fw-bold text-dark mb-1">Patient Lab Investigations</h4>
                        <p class="text-secondary small mb-0">Manage diagnostic reports and lab test requests.</p>
                    </div>
                    <a href="add_laboratory.php" class="btn btn-primary fw-semibold px-4 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2"><i class="bi bi-plus-circle-fill"></i> Add Lab Test</a>
                </div>
                <div class="table-responsive">
                    <table class="table align-middle mb-0">
                        <thead class="table-light text-secondary uppercase small">
                            <tr>
                                <th class="py-3 rounded-start">Test ID</th>
                                <th class="py-3">Patient ID</th>
                                <th class="py-3">Test Name / Details</th>
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <?php 
                                        // Fetch status from database dynamically
                                        $status_val = $row['status'] ?? 'Pending';
                                        
                                        // Assign badge colors based on test status
                                        $badge_class = 'bg-success text-success'; // Default to green for Completed
                                        if (strtolower($status_val) == 'pending') {
                                            $badge_class = 'bg-warning text-dark'; // Yellow for Pending
                                        } elseif (strtolower($status_val) == 'in progress') {
                                            $badge_class = 'bg-info text-info'; // Blue for In Progress
                                        }
                                    ?>
                                    <tr>
                                        <td class="fw-bold text-primary">#<?php echo $row['id']; ?></td>
                                        <td><?php echo htmlspecialchars($row['patient_id'] ?? 'N/A'); ?></td>
                                        <td><?php echo htmlspecialchars($row['test_name'] ?? $row['details'] ?? 'N/A'); ?></td>
                                        <td>
                                        ...
                                        <td class="text-end">
                                            <a href="laboratory.php?delete=<?php echo $row['id']; ?>" class="btn btn-sm btn-light border text-danger" onclick="return confirm('Delete this record?');"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="text-center py-5 text-muted small">
                                        <i class="bi bi-eyedropper fs-2 d-block mb-2 text-secondary opacity-50"></i>
                                        No laboratory test records found in database.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
