...
if (isset($_GET['delete'])) {
    ...
    header("Location: staff_payroll.php");
    exit();
}

$table_check = $conn->query("SHOW TABLES LIKE 'payroll'");
$result = null;
if ($table_check->num_rows > 0) {
    // Join payroll and staff tables to fetch staff name and dynamic values
    $result = $conn->query("SELECT p.*, COALESCE(s.name, 'N/A') as staff_name 
                            FROM payroll p 
...
<head>
    ...
    <title>Staff Payroll - Medi Lanka HMS</title>
...
<body>
    <nav class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="staff_payroll.php" class="sidebar-link active"><i class="bi bi-cash-stack"></i> Payroll & Staff</a>
    ...
    <div class="main-wrapper">
        <header class="top-navbar">
            <h5 class="fw-bold text-dark mb-0">Staff Payroll & Salaries</h5>
        ...
        <div class="p-4 p-md-5">
            <div class="content-card mb-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 class="fw-bold text-dark mb-1">Employee Payroll Management</h4>
                        <p class="text-secondary small mb-0">Manage hospital staff salaries, allowances, and payment records.</p>
                    </div>
                    <a href="add_payroll.php" class="btn btn-primary fw-semibold px-4 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2"><i class="bi bi-plus-circle-fill"></i> Add Payroll Record</a>
                </div>
                    ...
                    <table class="table align-middle mb-0">
                        <thead class="table-light text-secondary uppercase small">
                            <tr>
                                ...
                                <th class="py-3">Staff ID / Name</th>
                                <th class="py-3">Basic Salary (Rs.)</th>
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <?php 
                                        // Dynamic salary column check
                                        $salary_val = $row['basic_salary'] ?? $row['salary'] ?? $row['amount'] ?? 0;
                                        
                                        // Dynamic status badge styling
                                        $status_val = $row['status'] ?? 'Processed';
                                        $badge_class = 'bg-success text-success';
                                        if (strtolower($status_val) == 'pending') {
                                            $badge_class = 'bg-warning text-warning';
                                        } elseif (strtolower($status_val) == 'approved') {
                                            $badge_class = 'bg-info text-info';
                                        }
                                    ?>
                                    <tr>
                                        <td class="fw-bold text-primary">#<?php echo $row['id']; ?></td>
                                        <td><?php echo htmlspecialchars($row['staff_id'] . ' - ' . $row['staff_name']); ?></td>
                                        <td class="fw-semibold">Rs. <?php echo number_format((float)$salary_val, 2); ?></td>
                                        <td><span class="badge <?php echo $badge_class; ?> bg-opacity-10 px-2.5 py-1.5 fw-semibold"><?php echo htmlspecialchars($status_val); ?></span></td>
                                        <td class="text-end">
                                            <a href="staff_payroll.php?delete=<?php echo $row['id']; ?>" class="btn btn-sm btn-light border text-danger" onclick="return confirm('Delete this record?');"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="text-center py-5 text-muted small">
                                        <i class="bi bi-cash-stack fs-2 d-block mb-2 text-secondary opacity-50"></i>
