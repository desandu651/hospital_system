...
// Handle patient deletion if requested
...
$patients_result = $conn->query("SELECT * FROM patients ORDER BY id DESC");
?>
...
<head>
    ...
    <title>Patients Directory - Medi Lanka HMS</title>
    <!-- Google Fonts -->
...
<body>
    ...
    <nav class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="manage_patients.php" class="sidebar-link active"><i class="bi bi-people-fill"></i> Patients Directory</a>
    ...
    <div class="main-wrapper">
        ...
        <header class="top-navbar">
            <div class="d-flex align-items-center gap-2">
                <h5 class="fw-bold text-dark mb-0">Patients Directory</h5>
            </div>
        ...
        <div class="p-4 p-md-5">
            <div class="content-card mb-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 class="fw-bold text-dark mb-1">Registered Patients</h4>
                        <p class="text-secondary small mb-0">Manage and monitor hospital patient registry records.</p>
                    </div>
                    <a href="add_patient.php" class="btn btn-primary fw-semibold px-4 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2">
                            ...
                            <?php if ($patients_result && $patients_result->num_rows > 0): ?>
                                <?php while ($row = $patients_result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($row['gender'] ?? '-'); ?> / <?php echo htmlspecialchars($row['age'] ?? '-'); ?></td>
                                        <td><?php echo htmlspecialchars($row['phone'] ?? 'N/A'); ?></td>
                                        <td><?php echo htmlspecialchars($row['address'] ?? 'N/A'); ?></td>
                                        <td class="text-end">
                                            <a href="edit_patient.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-light border text-primary me-1"><i class="bi bi-pencil"></i></a>
                                            <a href="manage_patients.php?delete=<?php echo $row['id']; ?>" class="btn btn-sm btn-light border text-danger" onclick="return confirm('Are you sure you want to delete this patient record?');"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="text-center py-5 text-muted small">
                                        ...
                                        No patient records found in the database. Click "Add New Patient" to register.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
