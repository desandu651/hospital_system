...
$schedules_result = $conn->query("SHOW TABLES LIKE 'schedules'");
$result = null;
...
<head>
    ...
    <title>Doctor Schedules - Medi Lanka HMS</title>
...
<body>
    <nav class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="doctor_schedules.php" class="sidebar-link active"><i class="bi bi-clock-history"></i> Schedules</a>
    ...
    <div class="main-wrapper">
        <header class="top-navbar">
            <h5 class="fw-bold text-dark mb-0">Doctor Working Schedules</h5>
        ...
        <div class="p-4 p-md-5">
            <div class="content-card mb-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 class="fw-bold text-dark mb-1">Consultants Time Slots</h4>
                        <p class="text-secondary small mb-0">Manage clinical availability and visiting hours.</p>
                    </div>
                    <a href="add_schedule.php" class="btn btn-primary fw-semibold px-4 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2"><i class="bi bi-plus-circle-fill"></i> Add Schedule</a>
                </div>
                    ...
                    <table class="table align-middle mb-0">
                        <thead class="table-light text-secondary uppercase small">
                            <tr>
                                ...
                                <th class="py-3">Doctor ID / Name</th>
                                <th class="py-3">Day / Time Slot</th>
                            ...
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($row['doctor_id'] ?? 'N/A'); ?></td>
                                        <td><?php echo htmlspecialchars($row['day'] ?? $row['time_slot'] ?? 'N/A'); ?></td>
                                        <td><span class="badge bg-success bg-opacity-10 text-success px-2.5 py-1.5 fw-semibold">Active</span></td>
                                        <td class="text-end">
                                            <a href="doctor_schedules.php?delete=<?php echo $row['id']; ?>" class="btn btn-sm btn-light border text-danger" onclick="return confirm('Delete this schedule?');"><i class="bi bi-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="text-center py-5 text-muted small">
                                        ...
                                        No schedule records found in database.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
